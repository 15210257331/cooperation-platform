<template>
  <IterationFilter @on-search="handleSearch" @on-status-change="handleStatusChange"/>
  <div
  ...
  >
    <n-card v-for="item in iterationList" :key="item.id" :title="item.name" style="margin-bottom: 20px">
      <template #header-extra>
        <n-dropdown :options="dropdownOptions" trigger="hover" @select="handleDropDownClick">
          <n-icon :component="EllipsisHorizontal" />
        </n-dropdown>
      </template>
      <n-descriptions label-placement="left" :column="4">
        <n-descriptions-item label="迭代类型">
          <n-button text type="primary"> {{ item.type }} </n-button>
        </n-descriptions-item>
        ...
        <n-descriptions-item label="附件资源" :span="8">
          <n-space>
            <n-button
              v-for="(sonItem, index) in 5"
              :key="index"
              text
              size="tiny"
              type="primary"
            ...
            >
              ...
              需求评审文档2023年3月21日.pdf
            </n-button>
          </n-space>
        </n-descriptions-item>
        <n-descriptions-item label="迭代内容" :span="8">
          <ul>
            <li v-for="(sonItem, index) in item.content.split(',')" :key="index">{{ index + 1 }}、{{ sonItem }}</li>
          </ul>
        </n-descriptions-item>
      </n-descriptions>
    </n-card>
...
import { getIterationListAPI } from '@/api'
import { ref } from 'vue'
